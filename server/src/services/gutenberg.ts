...
export class GutenbergService {
  private static readonly BASE_URL = 'https://gutendx.com/books'
  private static readonly FALLBACK_BASE_URL = 'https://www.gutenberg.org'
  private static readonly MAX_TEXT_LENGTH = 8000 // Process only first 8k characters

  /**
   * Find the best .txt URL for a given book ID
   */
  static async resolveTextUrl(id: string): Promise<{ url: string; title: string; author: string; triedUrls?: string[] } | null> {
    const triedUrls: string[] = []
    
    try {
      // Try to get book metadata from Gutendx API first
      let book: GutenbergBook | null = null
      ...
      try {
        ...
        if (metadataResponse.ok) {
          book = await metadataResponse.json() as GutenbergBook
        }
      } catch (apiError) {
   ...
   */
  static async fetchText(id: string): Promise<{ text: string; title: string; author: string; wordCount: number; triedUrls?: string[]; source?: string } | null> {
   ...
   */
  private static async tryDirectGutenbergFetch(id: string): Promise<{ text: string; title: string; author: string; wordCount: number; triedUrls?: string[] } | null> {
   ...
   */
  private static async tryGutendxApproach(id: string): Promise<{ text: string; title: string; author: string; wordCount: number; triedUrls?: string[] } | null> {
   ...
   */
  private static async tryMockData(id: string): Promise<{ text: string; title: string; author: string; wordCount: number } | null> {
...
"From hell's heart I stab at thee; for hate's sake I spit my last breath at thee." - Captain Ahab`,
   ...
   */
  private static generateAllPossibleUrls(id: string): string[] {
    return [
   ...
   */
  private static async tryUrlWithRetries(url: string, id: string, triedUrls: string[], maxRetries: number = 2): Promise<{ text: string; title: string; author: string; wordCount: number } | null> {
    triedUrls.push(url)
   ...
   */
  private static async findWorkingTextUrl(formats: Record<string, string>, id: string, triedUrls: string[]): Promise<string | null> {
   ...
   */
  private static extractTextUrlsFromFormats(formats: Record<string, string>): string[] {
    const urls: string[] = []
   ...
   */
  private static async testUrl(url: string, triedUrls: string[]): Promise<boolean> {
    triedUrls.push(url)
   ...
   */
  private static extractMetadataFromText(text: string): { title?: string; author?: string } {
